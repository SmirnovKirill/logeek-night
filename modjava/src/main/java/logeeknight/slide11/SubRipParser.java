...
public class SubRipParser {
     ...
     * the text.
     */
    public static Subtitles from(String originalText) throws SubtitleFormatException {
        ...
        assertSubtitleNumber(lines[0]);
        Range<LocalTime> timeRange = getTimeRange(lines[1]);
        List<String> subtitleLines = new ArrayList<>();
        for (int i = 2; i < lines.length; i++) {
            if (isLineWithTimeRange(lines[i])) {
                assertBlank(lines[i - 2]);
                assertSubtitleNumber(lines[i - 1]);
                ...
                timeRange = getTimeRange(lines[i]);
                subtitleLines = new ArrayList<>();
            } else {
                subtitleLines.add(lines[i]);
            }
        }
...
class Subtitle {
    ...
    public Subtitle(LocalTime from, LocalTime to, List<String> lines) {
        this.from = from;
        this.to = to;
        this.lines = lines;
    }
...
class SubtitleFormatException extends Exception {
}